...
export const Categorays = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        console.log(fetchedPayments, " main data");
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
  ...
  const handleAddSelectedUsers = async () => {
    try {
      const response = await fetch(`${API}/categoray/addUsers`, {
        method: "POST",
        headers: {
                  ...
                  ({ customer, payments }) => (
                    <tr key={customer.id}>
                      ...
                      <td className="p-2 border min-w-[150px]">
                        <button onClick={handleDelete} className="link-dark d-inline-block mr-5" href="#">
            ...
            <tbody>
              {users.map((user) => (
                <tr key={user.id} className="border-t">
                  ...
                  <td className="py-2 px-4">{user.Gender === "ወ" ? "Male" : "Female"}</td>
                </tr>
              ))}
            </tbody>
          </table>
